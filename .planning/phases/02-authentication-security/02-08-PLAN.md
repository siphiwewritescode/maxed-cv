...
must_haves:
  ...
  artifacts:
    ...
    - path: "backend/src/auth/middleware/absolute-session-expiry.middleware.ts"
      provides: "Session expiry middleware using constant 7-day maxAge"
      min_lines: 15
  key_links:
...
**Note on authAPI.login type signature:** The backend LoginDto still accepts rememberMe as an optional field for backward compatibility, so TypeScript won't error when we remove it from the call. The backend simply ignores the rememberMe field if it's not provided.
  </action>
  <verify>
...
<task type="auto">
  <name>Task 3: Update backend login endpoint and middleware to always use 7-day session (remove 30-day rememberMe logic)</name>
  <files>backend/src/auth/auth.controller.ts, backend/src/auth/middleware/absolute-session-expiry.middleware.ts</files>
  <action>
**Root cause analysis:**
- Current: rememberMe extends cookie maxAge from 7 days to 30 days in auth.controller.ts
- Current: absolute-session-expiry.middleware.ts also checks rememberMe to enforce 7-day vs 30-day absolute expiry
- Requested: Everyone gets 7-day session (no 30-day extension)
- After removing rememberMe from login endpoint, the middleware's conditional logic becomes dead code

**Fix 1: auth.controller.ts - Remove rememberMe logic from login endpoint:**
...
**Fix 2: absolute-session-expiry.middleware.ts - Remove conditional rememberMe logic:**

Currently the middleware (lines 7-9) checks `req.session.rememberMe` to determine maxAge:
```typescript
...
After removing rememberMe from login, this becomes dead code. Update to use constant 7-day maxAge:
...
- Always enforces 7-day absolute session expiry
- No conditional logic based on rememberMe flag
- Simpler, more maintainable code
  </action>
  <verify>
Run `cd backend && npm run build` - should compile without errors.
Check that auth.controller.ts login endpoint removes rememberMe session flag and cookie extension logic.
...
4. **Middleware behavior:**
   - Check that absolute-session-expiry.middleware.ts has no rememberMe conditional logic
   - maxAge is constant 7 days for all sessions
   - Sessions older than 7 days are destroyed regardless of any rememberMe flag
...
- Absolute session expiry middleware uses constant 7-day maxAge (no conditional logic)
